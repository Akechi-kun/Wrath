...
internal static partial class Bozja
{
    ...
    public static bool IsInBozja => Content.TerritoryIntendedUse == IntendedUse.Bozja && (ContentCheck.IsInFieldOperations || ContentCheck.IsInFieldRaids);
            ...
            case JobRole.Tank:
                if (IsEnabled(Preset.Bozja_Tank))
                {
                    ...
                    foreach (var (preset, action, condition) in new[]
                    {
                        ...
                        (Preset.Bozja_Tank_LostCure, LostCure, PlayerHealthPercentageHp() <= Bozja_Tank_LostCure_Health),
                        (Preset.Bozja_Tank_LostArise, LostArise, GetTargetHPPercent() == 0 && !HasStatusEffect(RoleActions.Magic.Buffs.Raise)),
                        (Preset.Bozja_Tank_LostReraise, LostReraise, PlayerHealthPercentageHp() <= Bozja_Tank_LostReraise_Health),
